// CareerRai Phase 2 seed — 30 days of rich, realistic data
...
async function main() {
  console.log('🌱 CareerRai Phase 2 — seeding rich 30-day data...\n');
  ...
  for (const email of studentEmails) {
    ...
    if (!sid) { console.log(`  ⚠ ${email} not found`); continue; }
  ...
  fbRows.push(
    ...
    { buddy_id: buddy1Id, student_id: byEmail['priya@careerrai.com'], feedback_date: dateStr(5), feedback_text: "Priya — stress is at 4.5 this week. I'm flagging this. Study hours are high but quality is low. Take 2 days complete off. Seriously. Then restart with a lighter week.", rating: 2, next_steps: ['Take 2 days complete rest', 'Restart with lighter 3h sessions', 'Schedule 1:1 call'], period_covered: 'adhoc' },

    { buddy_id: buddy1Id, student_id: byEmail['rohan@careerrai.com'], feedback_date: dateStr(28), feedback_text: "Rohan, CUET prep is different from CAT — you need domain subject consistency. Right now you're spending too much on General Studies. Flip the ratio: 60% domain, 40% GS.", rating: 3, next_steps: ['Increase domain subject practice', 'Reduce GS to 40% of prep time'], period_covered: 'weekly' },
    { buddy_id: buddy1Id, student_id: byEmail['rohan@careerrai.com'], feedback_date: dateStr(21), feedback_text: "4 missed days in 2 weeks. What's happening? I saw your notes — 'family stuff'. That's valid but we need to build back up. Can you commit to at least 2h on busy days?", rating: 2, next_steps: ['Commit to minimum 2h even on hard days', 'Share daily schedule with me'], period_covered: 'weekly' },
    { buddy_id: buddy1Id, student_id: byEmail['rohan@careerrai.com'], feedback_date: dateStr(10), feedback_text: "Big improvement this week — back to 5+ days, study hours up. Mock improved to 71%. This is the Rohan I knew was there. Keep the momentum, don't let up.", rating: 4, next_steps: ['Maintain 5+ days consistency', 'Push mock frequency to every 4 days'], period_covered: 'weekly' }
  );

  // Priya Mentor's feedback for Meera and Arjun
  fbRows.push(
    { buddy_id: buddy2Id, student_id: byEmail['meera@careerrai.com'], feedback_date: dateStr(28), feedback_text: "Meera, excellent week 1. Your Verbal scores are naturally high — focus energy on Quant to balance. I'll send you a Quant topic list to prioritize.", rating: 4, next_steps: ['Prioritize Quant practice', 'Attempt Quant sectional tests'], period_covered: 'weekly' },
    { buddy_id: buddy2Id, student_id: byEmail['meera@careerrai.com'], feedback_date: dateStr(21), feedback_text: "Very consistent — 6/7 days, mock improving. You seem to be in a good rhythm. Keep this up through the next 3 weeks and you'll be exactly where the top percentile students are.", rating: 5, next_steps: ['Maintain consistency', 'Add one extra mock per week'], period_covered: 'weekly' },
    { buddy_id: buddy2Id, student_id: byEmail['meera@careerrai.com'], feedback_date: dateStr(7), feedback_text: "⚠️ Red flag — you've missed 3 days this week and stress has jumped to 4.5. What's going on? Please DM me today. This is not a number issue, this is a you issue. I'm here.", rating: 2, next_steps: ['Immediate 1:1 call', 'Understand root cause of stress spike', 'Light revision only this week'], period_covered: 'adhoc' },

    { buddy_id: buddy2Id, student_id: byEmail['arjun@careerrai.com'], feedback_date: dateStr(28), feedback_text: "Arjun, welcome! Week 1 looks understandably rough — CUET prep is new for most students. Goal for now: just fill the daily report. Don't worry about hours, build the habit first.", rating: 3, next_steps: ['Build daily report habit first', 'Start with 2h sessions'], period_covered: 'weekly' },
    { buddy_id: buddy2Id, student_id: byEmail['arjun@careerrai.com'], feedback_date: dateStr(14), feedback_text: "Improvement! 5/7 days now and hours climbing. Your domain subject scores are naturally good. Focus on English next — that's the differentiator in CUET. Start RC practice daily.", rating: 3, next_steps: ['Daily RC practice', 'Work on English proficiency'], period_covered: 'weekly' },
    { buddy_id: buddy2Id, student_id: byEmail['arjun@careerrai.com'], feedback_date: dateStr(3), feedback_text: "Good trend over 4 weeks — you've found your rhythm. Confidence is up, stress is down. Now push: 4h minimum daily and one sectional test every 3 days.", rating: 4, next_steps: ['Push to 4h minimum daily', 'Sectional test every 3 days'], period_covered: 'weekly' }
  );
  ...
  if (validFb.length > 0) {
    ...
    else console.log(`  ✓ ${validFb.length} feedback entries created`);
  }

  // ── Test Results ───────────────────────────────────────────────
  console.log('\nCreating test results...');
  const testRows = [
    // Aarav — CAT, consistently improving
    ...
    // Priya — CAT, high stress, flat/declining
    ...
    // Rohan — CUET, inconsistent but recovering
    ...
    // Meera — CAT, was improving now declining
  ...
  if (testErr) console.error('  ✗ Test results error:', testErr.message);
  else console.log(`  ✓ ${testRows.length} test results created`);
  ...
  notifRows.push(
    { user_id: byEmail['aarav@careerrai.com'], type: 'feedback_received', title: 'Nishant left you feedback 🎯', body: 'Mock score jumped from 68→75 — that\'s real momentum. Check your buddy feedback.', read: false, channel: 'in_app', data: {} },
    { user_id: byEmail['aarav@careerrai.com'], type: 'streak_milestone', title: '🔥 7-day streak!', body: 'You\'ve filled reports 7 days in a row. Elite consistency. Keep it alive.', read: true, channel: 'in_app', data: {} },
    ...
    { user_id: byEmail['priya@careerrai.com'], type: 'feedback_received', title: 'Nishant left you urgent feedback ⚠️', body: 'Stress is at 4.5 this week. Nishant has a recovery plan for you — read it now.', read: false, channel: 'in_app', data: {} },
    { user_id: byEmail['priya@careerrai.com'], type: 'daily_reminder', title: 'Report pending 📋', body: 'Hey Priya, your daily report is pending. Takes 90 seconds — do it now.', read: false, channel: 'in_app', data: {} },

    { user_id: byEmail['rohan@careerrai.com'], type: 'feedback_received', title: 'Nishant left you feedback 📈', body: 'Big improvement this week! Mock improved to 71%. Check what Nishant says next.', read: false, channel: 'in_app', data: {} },

    { user_id: byEmail['meera@careerrai.com'], type: 'feedback_received', title: 'Priya M flagged a concern ⚠️', body: 'You\'ve missed 3 days and stress is high. Your buddy wants to talk. Check feedback.', read: false, channel: 'in_app', data: {} },

    { user_id: byEmail['arjun@careerrai.com'], type: 'feedback_received', title: 'Priya M left you feedback 🎯', body: 'Good trend over 4 weeks — you\'ve found your rhythm! Read your weekly feedback.', read: false, channel: 'in_app', data: {} }
  );

  // Buddy notifications
  notifRows.push(
    { user_id: buddy1Id, type: 'red_flag', title: '⚠️ Red flag: Priya Verma', body: 'Priya\'s stress hit 4.5 avg this week. Mock scores declining. Needs intervention.', read: false, channel: 'in_app', data: { student_email: 'priya@careerrai.com' } },
    { user_id: buddy1Id, type: 'weekly_digest', title: 'Weekly digest — 3 students', body: 'Aarav: On track (88/100) • Priya: Needs intervention (42/100) • Rohan: Needs nudging (58/100)', read: false, channel: 'in_app', data: {} },
    { user_id: buddy1Id, type: 'report_submitted', title: 'Aarav submitted today\'s report ✓', body: '5.8h study, mock score 82%. On a 7-day streak. Looking strong.', read: true, channel: 'in_app', data: {} },

    { user_id: buddy2Id, type: 'red_flag', title: '⚠️ Red flag: Meera Patel', body: 'Meera has missed 3 consecutive days. Stress at 4.5. Immediate check-in recommended.', read: false, channel: 'in_app', data: { student_email: 'meera@careerrai.com' } },
    { user_id: buddy2Id, type: 'weekly_digest', title: 'Weekly digest — 2 students', body: 'Meera: Needs intervention (38/100) • Arjun: Needs nudging (55/100)', read: false, channel: 'in_app', data: {} }
  );
  ...
  if (validNotifs.length > 0) {
    ...
    else console.log(`  ✓ ${validNotifs.length} notifications created`);
  }

  console.log('\n✅ Phase 2 seed complete!');
  console.log('\nStudent patterns seeded:');
  console.log('  Aarav   — consistent_improver  (mock 68→82, on track)');
  console.log('  Priya   — stressed_hardworker  (high stress, declining mocks, needs intervention)');
  console.log('  Rohan   — inconsistent_recovering (was dropping, now recovering)');
  console.log('  Meera   — declining_redflag    (good then disappeared last week)');
  console.log('  Arjun   — new_finding_rhythm   (slowly building up)');
}
