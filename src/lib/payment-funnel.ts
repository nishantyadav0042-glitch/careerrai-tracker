...
/**
 * The metadata key an order id is stored under. ONE spelling, imported by both
 * ends, because the two ends had two spellings and nobody could see it.
 *
 * The ingestion route wrote `order_id`; revenue-ops read `orderId`. Measured 17
 * Sep across 87 production rows: 42 carried `order_id`, **0 carried `orderId`**.
 * So the join that decides whether an abandoned checkout is our defect or the
 * student's decision could never match a single row, and every abandoned order
 * fell through to "never reached Razorpay" — a false accusation against our own
 * product, generated by the code written to stop false accusations.
 *
 * A literal string on each side is what made that invisible. Anything that
 * stores or reads this key must use this constant.
 */
export const FUNNEL_ORDER_KEY = 'order_id';

/** Read an order id out of a stored funnel event's metadata. */
export function funnelOrderId(metadata: unknown): string | null {
  const v = (metadata as Record<string, unknown> | null)?.[FUNNEL_ORDER_KEY];
