...
export async function getStudent360(admin: any, id: string): Promise<Student360 | null> {
  ...
  const [{ data: p }, momentum, { data: reports }, { data: notifs }, { data: eng }, { data: grants }, { data: winRoutines }, { data: winCompletions }, { data: coverageRows }, { data: debriefs }, decisions] = await Promise.all([
    ...
    admin.from('daily_reports').select('report_date, created_at, study_duration, plan_fit, mock_taken, day_outcome, study_duration_source').eq('student_id', id).order('report_date', { ascending: false }),
  ...
  // Q4 — a day we never measured is not evidence of behaviour. It contributes
  // nothing to `hrs` already (the engine filters h > 0), but it must not count
  // toward MIN_DAYS_FOR_BEHAVIOUR either, or a student gets judged against
  // their own stated hours on the strength of days we never asked about.
  // A real zero still counts: that IS behaviour.
  ...
  // A3 — the student's declared outcome counts as an active day even when the
  // hours column is 0 (a check-in never asks for hours). `hrs` above is left
  // alone deliberately: it feeds computeCapacity, a deferred consumer.
