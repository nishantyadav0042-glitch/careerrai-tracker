...
import {
  ...
  closeCopy, planMorningCopy, lessonLinkAnnounceCopy,
...
describe('student-facing notification copy never says "log"', () => {
  it('holds for every fixed-argument slot', () => {
    ...
    assertNoLog(lessonLinkAnnounceCopy(planMorningCopy('Aarav', 'Geometry', 'RC', 4, 2.5), 'Aarav'), 'lessonLinkAnnounceCopy');
...
describe('notifications report work done — they never chase', () => {
  it('holds for every fixed-argument slot', () => {
    ...
    assertNoChasing(lessonLinkAnnounceCopy(planMorningCopy('Riya', 'Percentages', 'RC', 3, 2), 'Riya'), 'lessonLinkAnnounceCopy');
