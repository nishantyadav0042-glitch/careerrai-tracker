...
function buildMessage(objective: Objective, first: string, ctx: MessageCtx): string {
  ...
  switch (objective) {
    ...
    case 'buddy':
      return `${first}, ab sirf ${d} din bache hain. You looked at Exam Buddy and left it. Nishant here, I built CareerRai, so let me be straight with you. Book one 45-minute session with an IIM buddy — wo tumhara actual prep padh ke aate hain, batate hain score kahan atka hai, aur you leave with a written next step. Rs 299 + GST, one time — aur full plan loge within 7 days to Rs 299 usme adjust ho jata hai. In ${d} dino me guesswork band karo, ek session karke dekho. ${SITE_URL}/student/buddy`;
