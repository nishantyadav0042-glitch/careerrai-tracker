...
export async function loadMasteryState(
  ...
  const { data: rows } = await admin.from('qa_topic_progress').select('*').eq('student_id', studentId).eq('section', section);
  ...
  if ((rows ?? []).length === 0) {
    ...
    const { data: coverage } = await admin.from('topic_coverage').select('topic, status, updated_at').eq('student_id', studentId);
    const seedRows: any[] = [];
    ...
    if (seedRows.length > 0) await admin.from('qa_topic_progress').upsert(seedRows, { onConflict: 'student_id,section,topic' });
  } else {
