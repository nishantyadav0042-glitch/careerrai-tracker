  ...
  const handleSubmit = async () => {
    ...
    try {
      const { error: insertError } = await supabase.from('daily_reports').upsert({
        ...
        quality_focus: parseValue(qualityFocus) ?? 3,
        difficulty: parseValue(difficulty) ?? 3,
        mock_taken: mockTaken === true || mockTaken === 'true',
        mock_name: (mockTaken === true || mockTaken === 'true') && mockName ? mockName : null,
        quant_score: (mockTaken === true || mockTaken === 'true') ? parseValue(quantScore) : null,
        verbal_score: (mockTaken === true || mockTaken === 'true') ? parseValue(verbalScore) : null,
        logic_score: (mockTaken === true || mockTaken === 'true') ? parseValue(logicScore) : null,
        total_accuracy: (mockTaken === true || mockTaken === 'true') ? parseValue(totalAccuracy) : null,
        confidence: parseValue(confidence) ?? 3,
        stress: parseValue(stress) ?? 3,
        sleep_quality: parseValue(sleepQuality) ?? 3,
        nutrition_exercise: nutritionExercise === true || nutritionExercise === 'true',
        overall_energy: parseValue(overallEnergy) ?? 3,
        notes: notes || null,
      });
