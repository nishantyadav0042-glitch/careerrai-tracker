...
export interface DetectiveCaseContent {
...
export function isDetectiveCase(content: unknown): content is DetectiveCaseContent {
  const c = content as Partial<DetectiveCaseContent> | null | undefined;
  return (
    !!c &&
...
export function DetectiveCaseModal({
  ...
  const caseNumber = caseDate.replace(/-/g, '').slice(2); // e.g. 260612
  ...
  const handleNextQuestion = async () => {
    ...
    // Close the case
    const correct = qResults.filter(Boolean).length;
    const timeSeconds = Math.max(1, Math.floor((Date.now() - startRef.current) / 1000));
    setPhase('closed');
    setSaving(true);
    try {
      await onComplete({
        solved: correct >= Math.ceil(questions.length / 2),
        timeSeconds,
  ...
  const stars =
    correctAnswers === questions.length && hintsUsed === 0 && checksUsed <= 3
      ? 3
      : correctAnswers >= Math.ceil(questions.length / 2)
      ? 2
      : 1;
  const rankLabel = stars === 3 ? 'Ace Detective' : stars === 2 ? 'Inspector' : 'Rookie';

  // Circular slot position (Seat 1 at top, clockwise)
  ...
  const renderSlotChip = (i: number) => (
    <button
      onClick={() => handleSlotTap(i)}
      className={cn(
        'w-full min-h-[34px] rounded-lg border-2 text-[11px] font-semibold px-1 py-1.5 transition-all leading-tight',
        placements[i]
          ? boardCracked
            ? 'border-emerald-500 bg-emerald-50 text-emerald-800'
            : 'border-amber-500 bg-amber-50 text-amber-900'
          : selected
          ? 'border-dashed border-amber-400 bg-amber-50/50 text-stone-400 animate-pulse'
  ...
  return (
    <div className="fixed inset-0 bg-stone-900/80 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
      <div className="bg-white w-full sm:max-w-lg rounded-t-2xl sm:rounded-2xl shadow-2xl max-h-[94vh] overflow-y-auto">
        {/* Header */}
        <div className="sticky top-0 z-10 bg-stone-900 text-white px-5 py-3.5 flex items-center justify-between rounded-t-2xl">
          <div>
            <span className="text-[10px] uppercase tracking-widest font-semibold text-amber-400">
              🕵️ Case File #{caseNumber}
            </span>
          ...
          <div className="p-5 space-y-5">
            ...
            <div className="grid grid-cols-3 gap-2 text-center text-[11px] text-stone-600">
              <div className="bg-stone-50 rounded-lg p-2.5 border border-stone-200">
                <p className="font-bold text-stone-900">{content.entities.length}</p>
                <p>suspects</p>
              </div>
              <div className="bg-stone-50 rounded-lg p-2.5 border border-stone-200">
                <p className="font-bold text-stone-900">{content.clues.length}</p>
                <p>evidence</p>
              </div>
              <div className="bg-stone-50 rounded-lg p-2.5 border border-stone-200">
                <p className="font-bold text-stone-900">{questions.length}</p>
                <p>case questions</p>
              </div>
            </div>
            <button
          ...
          <div className="p-5 space-y-5">
            ...
            <div>
              <p className="text-[10px] uppercase tracking-widest font-bold text-stone-500 mb-2">
                📌 Evidence board — tap a suspect, then a position
              </p>
              {content.mode === 'circular' ? (
                <div className="relative w-full max-w-[300px] aspect-square mx-auto">
                  ...
                  {content.slotLabels.map((label, i) => (
                    <div
                      key={i}
                      className="absolute w-[72px] -translate-x-1/2 -translate-y-1/2 flex flex-col items-center gap-0.5"
                      style={circlePos(i)}
                    >
                  ...
                  {trayEntities.map((e) => (
                    <button
                      ...
                      className={cn(
                        ...
                        selected === e
                          ? 'border-amber-500 bg-amber-100 text-amber-900 scale-105 shadow'
            ...
            <div>
              <p className="text-[10px] uppercase tracking-widest font-bold text-stone-500 mb-2">
                🗂 Evidence — tap to strike off once used
              </p>
              <div className="space-y-1.5">
                {content.clues.slice(0, unlockedClues).map((clue, i) => (
                  <button
                    ...
                    className={cn(
                      'w-full text-left px-3 py-2 rounded-lg border text-xs leading-relaxed transition-all',
                      struckClues.includes(i)
                        ? 'border-stone-200 bg-stone-50 text-stone-400 line-through'
                        : 'border-amber-200 bg-amber-50 text-stone-800'
                    )}
                  >
                    <span className="font-bold text-amber-700 mr-1.5">#{i + 1}</span>
                    {clue}
                  </button>
                ))}
                {unlockedClues < content.clues.length && (
                  <button
                    onClick={() => setUnlockedClues((n) => n + 1)}
                    className="w-full flex items-center justify-center gap-2 px-3 py-2 rounded-lg border-2 border-dashed border-stone-300 text-xs font-semibold text-stone-600 hover:border-amber-400 hover:text-amber-700 transition-all"
                  >
                    <Lock className="w-3.5 h-3.5" />
                    Unlock next evidence ({content.clues.length - unlockedClues} left)
                  </button>
                )}
              </div>
            </div>
            ...
            {lastCheck && !boardCracked && (
              <div className="bg-rose-50 border border-rose-200 rounded-xl p-3 text-sm text-rose-800 font-medium">
                🎯 {lastCheck.correct} of {lastCheck.total} positions correct — keep deducing!
              </div>
            )}
            {boardCracked && (
              <div className="bg-emerald-50 border-2 border-emerald-300 rounded-xl p-4 text-center">
                <p className="text-sm font-bold text-emerald-800">
                  🎉 Theory confirmed! The arrangement is cracked.
                </p>
                <p className="text-xs text-emerald-700 mt-1">Now close the case…</p>
              </div>
            )}
            ...
            {!boardCracked && (
              <div className="flex gap-2">
                <button
                  ...
                  className="flex items-center justify-center gap-1.5 px-3 py-2.5 rounded-xl border-2 border-stone-200 text-xs font-semibold text-stone-600 hover:border-amber-400 disabled:opacity-40 transition-all"
                >
                ...
                <button
                  onClick={handleReset}
                  className="flex items-center justify-center gap-1.5 px-3 py-2.5 rounded-xl border-2 border-stone-200 text-xs font-semibold text-stone-600 hover:border-stone-400 transition-all"
                >
          ...
          <div className="p-5 space-y-5">
            ...
            <div className="flex items-center justify-between">
              <p className="text-[10px] uppercase tracking-widest font-bold text-stone-500">
                ⚖️ Close the case — Question {qIndex + 1} of {questions.length}
              </p>
                ...
                return (
                  <button
                    ...
                    className={cn(
                      ...
                      !qSubmitted && isPicked && 'border-amber-500 bg-amber-50 text-stone-900',
                  ...
                  >
                    <span className="font-mono mr-2 text-stone-500">
            ...
            {!qSubmitted ? (
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-1 py-3 bg-amber-600 text-white rounded-xl font-semibold text-sm hover:bg-amber-700 transition-all active:scale-[0.98]"
              >
                {qIndex + 1 < questions.length ? 'Next question' : 'Close the case'}
        ...
        {phase === 'closed' && (
          <div className="p-5 space-y-5">
              ...
              <div className="flex items-center justify-center gap-1">
                {[1, 2, 3].map((s) => (
                  <Star
                    ...
                    className={cn(
                      'w-7 h-7',
                      s <= stars ? 'text-amber-400 fill-amber-400' : 'text-stone-200'
                    )}
                  />
                ))}
              </div>
              <p className="text-sm font-semibold text-amber-700">Rank earned: {rankLabel}</p>
            </div>

            <div className="grid grid-cols-3 gap-2 text-center">
              <div className="bg-stone-50 rounded-xl p-3 border border-stone-200">
                <p className="text-base font-bold text-stone-900">{mins}:{secs}</p>
                <p className="text-[10px] text-stone-500 mt-0.5">Time</p>
              </div>
              <div className="bg-stone-50 rounded-xl p-3 border border-stone-200">
                <p className="text-base font-bold text-stone-900">
                  {correctAnswers}/{questions.length}
                </p>
                <p className="text-[10px] text-stone-500 mt-0.5">Questions</p>
              </div>
              <div className="bg-stone-50 rounded-xl p-3 border border-stone-200">
                <p className="text-base font-bold text-stone-900">{checksUsed}</p>
                <p className="text-[10px] text-stone-500 mt-0.5">Theory checks</p>
              </div>
            </div>
            ...
            <p className="text-center text-xs text-stone-500">
              This was a real CAT-style LRDI set. Come back tomorrow for a new case. 🔍
            </p>
