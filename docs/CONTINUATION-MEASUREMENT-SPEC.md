...
## SUPERSEDED IN PART — 23 Sep, by PR #214

This audit concluded on 22 Sep that the Day-1 → Day-2 boundary was structurally
unobservable. **That was true when written and is no longer fully true.** PR
#214 (`138f6e74`, merged to main 23 Sep) shipped `src/lib/session-boundary.ts`,
`src/lib/os/day-bridge.ts` and an `app_resume` event — which is the
SESSION_ENDED signal and part of the RE-ENTRY signal this audit specified.

What still stands: the measured shape of the bridge (median gap 1 day, 42 of 70
repeat learners studying again the very next day), the 31.4% of repeat learners
whose last Day-1 act cannot be named from the old data, and the withdrawal of
the `daily_routines`-existence measure as circular.

What must NOT be carried forward unchecked: any claim that the boundary cannot
be observed. Re-measure against the new signals before quoting this section.

Two of the four signals specified here — SESSION_ENDED and, partly, RE-ENTRY —
were implemented by PR #214 before this spec was merged. Treat §2.2 and §2.4 as
a record of the requirement, not as work outstanding. RESUME | RESTART (§2.5)
and STATE_LEFT_BEHIND (§2.3) are, as far as this session can see, still open —
verify against `src/lib/os/day-bridge.ts` before acting on either.
