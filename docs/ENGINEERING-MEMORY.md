...
| 17 | 2026-08-04 | One meeting, two truths — mentor locked out of the call | Trust | 1st orientation |
...
## Incident #17 — one meeting, two truths: the mentor lost the Join button at T+0

- **Date:** 2026-08-04 · **Area:** Trust OS · **Severity:** P1
- **Impact:** The first paid orientation failed at the moment it began. The
  student joined an empty room ("Hello I had joined the call, nobody was in
  the meet!"); her mentor could not get in ("Yes joining my app was stuck").
- **Root cause:** the student's session list queried `scheduled_at >= now - 1h`
  (a grace window, with a comment explaining why). Every buddy-side surface
  queried `scheduled_at >= now`. At the scheduled second the row dropped out of
  the mentor's query, so the session — and its Join button — vanished from her
  app entirely. Combined with a `minsAway <= 15` gate, a mentor's entire join
  window was T-15min to T-0, slamming shut exactly when the call started.
- **How it hid:** Daily.co was healthy (room valid, key live, billing fine), so
  every check of "is video working" passed. The failure was in *who could see
...
- **Lessons:**
  1. **Two people in one meeting must never get two different answers to "is
     ...
     different queries) — here it was the same session from different queries.
  2. **A grace window added on one side of a two-sided feature is a bug on the
     other side.** The fix was applied where the symptom was reported.
  3. **Verify end-to-end from every seat**, not just the one that complained.
- **Prevention (encoded):** `src/lib/session-window.ts` — `sessionsVisibleFrom()`
  ...
  (student home, student buddy page, buddy profile, buddy student detail).
  `logged-out-routing.test.ts` pins the join window, including the exact cases
  that used to fail (`isJoinOpen(0)`, `isJoinOpen(-4)`).
- **Owner:** cofounder (AI)
...
- **Impact:** A buddy with a paying student assigned could not reach her own
  ...
  reported "this link is taking me to only student portal now." She had never
...
- **How it hid:** `/start` carries a prominent Log in link *in triplicate*,
  ...
  `/start`, and never inherited the rule. The door was fixed one level too deep.
- **Lessons:**
  1. **This is Incident #10 again.** An unreachable login already cost a store
     ...
     whatever page is the front door.
  2. **When a new screen is placed in front of an old one, it inherits the old
     one's obligations** — the front door moved, the lock did not.
...
- **Prevention (encoded):** `/welcome` carries the login door with a
  ...
  store-launch holdback that Play review depends on.
- **Owner:** cofounder (AI)
