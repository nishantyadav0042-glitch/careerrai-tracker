...
| 72 | 2026-09-05 | Production reported 27 students on a 3+ day streak and 4 above seven days. Rebuilt from `daily_reports`, the real numbers were **8 and ZERO** — students who had last logged 42, 23 and 21 days ago were still counted as active streaks. `streak_data.current_streak` is written at log time and NEVER decays, so the stored value means "the streak this student HAD at their last log", never "the streak they have". `liveStreak()` had existed since 20 July with the rule in its own docblock — *"Every DISPLAY of a streak must go through this"* — and nothing enforced it, so it eroded into ten files: the student's own logging screen, the daily routine payload, the 08:00 morning push, the mentor's suggestion list (which read it backwards, hiding "log every day" from exactly the students who needed it), the sales-ready criterion, and the intervention ledger's before/after measurement. Two AI-draft producers (`buddy-briefing`, `feedback-draft`) additionally read the DEAD `profiles.current_streak` — 0 for every student — the same bug `chat/draft` had already fixed and nobody propagated. Fixed by routing every read through `liveStreak`, pointing both drafts at `streak_data`, and adding `streak-decay.guard.test.ts` so the rule is executable rather than documented | Retention/Streaks (P1) | every student shown a streak they no longer had, every mentor briefed on a false one, and the founder's own retention read — the number we judge the product by |
