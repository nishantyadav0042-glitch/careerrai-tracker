# Event OS — Notification & Identity Architecture v1.1 (BINDING)

> Locked 26 Aug 2026 after two independent review passes (Claude research ×3,
> external founder-side review ×2) were reconciled line-by-line. This is the
...
4. **No message body text in WhatsApp templates** (privacy: shared family
   ...
   percentile.
5. **No account without a verified phone.** Google (and later Apple) opens the
...
| Event | Sole producer | Idempotency key | Ladder (per capability) | Phase |
|---|---|---|---|---|
| SESSION_BOOKED | booking route via dispatch() | session_id+"booked" | WA → push → in-app; calendar minted both sides | 2 |
| SESSION_REMINDER_24H | session cron | session_id+"24h" | calendar if connected, else WA → push | 2 |
| SESSION_REMINDER_30M | session cron | session_id+"30m" | WA → push (join link) | 2 |
| SESSION_CANCELLED / RESCHEDULED | cancel/reschedule routes | session_id+state-version | WA → push; calendar updated; bypasses quiet hours | 2 |
| MESSAGE_RECEIVED (student→buddy) | chat send route | message_id (collapse: thread) | WA → push; deep link; no body | 2 |
| MESSAGE_REPLIED (buddy→student) | chat send route | message_id (collapse: thread) | WA → push; highest-value retention trigger | 2 |
| SESSION_COMPLETED → FEEDBACK_REQUEST | completion route | session_id+"feedback" | WA/push → in-app rating | 3 |
| GOOD_RATING → TESTIMONIAL_ASK | feedback handler | session_id+"testimonial" | only after positive rating; bad rating → recovery, never an ask | 3 |
| BUDDY_SLA_BREACH | SLA cron | message_id+"sla" | WA to buddy; admin on 2nd breach; never nags the student | 4 |
| DAILY_NUDGE | existing ladder crons | user+IST-day+type (live index) | push ONLY (invariant 2) | 4 |
| WEEKLY_INSIGHT_DIGEST | weekly cron | user+ISO-week | email → push; behaviour-powered — the moat notification | 4 |
| BUDDY_PITCH | claim authority | promo_impressions PK | unchanged, live | live |
